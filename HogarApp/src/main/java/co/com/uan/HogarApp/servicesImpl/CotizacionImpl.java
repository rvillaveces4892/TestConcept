...
public class CotizacionImpl implements ICotizacion {
	...
	public boolean aceptarCotizacion(Long solicitud_id, Long cotizacion_id) {
		try {
			em.createNativeQuery("UPDATE COTIZACION SET ESTADO = 'RECHAZADA' WHERE SOLICITUD_ID = ? AND COTIZACION_ID != ?")
			.setParameter(0, solicitud_id).setParameter(1, cotizacion_id);
			em.createNativeQuery("UPDATE COTIZACION SET ESTADO = 'ACEPTADA' WHERE COTIZACION_ID = ?")
			.setParameter(0, cotizacion_id);
			return true;
		} catch (Exception e) {
			e.printStackTrace();
			return false;
	...
	public boolean rechazarCotizacion(Long solicitud_id, Long cotizacion_id) {
		try {
			em.createNativeQuery("UPDATE COTIZACION SET ESTADO = 'RECHAZADA' WHERE COTIZACION_ID = ?")
			.setParameter(0, cotizacion_id);
			return true;
		} catch (Exception e) {
			e.printStackTrace();
